...
class Login:
	...
	def send_captch(self):#发送验证码请求，正常情况下不需要验证码
		...
		if login_for_captch.text!="false":
			Print().Green_print("操作频繁导致需要验证码，请稍后重试")
	...
	def get_status(self): #爬取基本信息
		...
		for li in subSelfModeRT.find_all('li'):
			...
			ip=subSelfModeRTList[0].text.replace('\t\t\t\t','') #获取在线设备ip
			name=subSelfModeRTList[1].text.replace("\n","").replace("					","").replace('\r','').replace('\t\t\t\t','') #获取在线设备名称
...
class Print:
	...
	def account_info(self):
		print("学号："+username)
		print("账号有效期："+account_remaining_time)
		print("服务截止时限："+service_remaining_time)
		print("当前设备ip："+PresentIP)
		print("计费政策："+policyM)
		print("最大连接数："+maxOnlineNum)
  ...
  \\ V  V /  __/ | (_| (_) | | | | | |  __/
   \\_/\\_/ \\___|_|\\___\\___/|_| |_| |_|\\___|''')
		if policyM=='学生国际畅游包月':
			...
			call="老师"
		print(BOLD+user_realname+call+END_COLOR+','+GREEN+"北大医学欢迎您回家！"+END_COLOR)
	def status(self): #打印设备在线状态
		...
		for ip in online_ip:
			print("设备状态：在线")
			print("设备ip："+ip)
			print("设备mac地址："+online_mac[i])
			print("设备名称："+online_name[i])
			if online_bind_flag[i]=='1':
				print("该设备已绑定mac（原则上未来免认证）")
			elif online_bind_flag[i]=='0':
				print("该设备未绑定mac（原则上未来需重新认证）")
			i=i+1
			Print().Green_print("-"*50)
		i=0
		for ip in offline_ip:
			print("设备状态：离线")
			print("设备ip："+ip)
			print("设备mac地址："+offline_mac[i])
			print("设备名称："+offline_name[i])
	...
	def wait_time_axis(self,seconds):
		t=seconds/100
		Print().Green_print("等待服务器刷新，"+"等待时间："+str(seconds)+"秒")
		for i in range(101):
			time.sleep(t)
...
class Command:
	...
	def auth_by_ip(self,ip,bind_mac_flag): #flag为0不绑定mac，flag为1绑定mac
		...
		if bind_mac_flag=='0':
			bind_tag="未来需密认证"
		elif bind_mac_flag=='1':
			bind_tag="未来免密认证"
		Print().wait_time_axis(6)
		Login().get_status()
		if ip!='' and ip in online_ip:
			ip_tag="ip地址为"+ip+"的设备认证成功，"
			Print().Green_print(ip_tag+"认证方式为"+bind_tag)
		elif ip!='' and ip not in online_ip:
			ip_tag="ip地址为"+ip+"的设备认证失败"
	...
	def drop_all(self):#全部下线函数
		...
		Login().get_status()
		if online_ip==[]:
			Print().Green_print("所有设备下线成功")
		else:
			Print().Green_print("下线所有设备失败，请稍后重试")
	'''
...
class Menu:
	#交互界面函数类
	def main(self): #主页面
		Print().SkyBlue_print("当前位置：首页")
		choose_in_main_menu=input("请按数字键选择您的服务\n0、退出程序\n1、设备管理\n2、设备认证\n3、状态刷新\n4、切换至英文版\n")
		if choose_in_main_menu not in {'0','1','2','3','4'}:
				Print().Red_print("输入有误，重新输入")
				self.main()
		else:
			if choose_in_main_menu=='0':
				print(GREEN+"北大医学祝您校园网使用愉快"+END_COLOR+RED+"\n故障申报热线：010-82802999"+END_COLOR)
				os._exit(0) #结束程序
				...
				Login().get_status()
				Print().Green_print("状态刷新成功")
				self.main()
			elif choose_in_main_menu=='4':
				PKUHSC_Gateway_en_us.main_en_us()
	def manage(self): #设备管理界面
		Print().SkyBlue_print("当前位置：首页>>设备管理")
		choose_in_manage=input("请按数字键选择您的服务\n0、退出程序\n1、下线设备\n2、解绑设备\n3、设备重命名\n9、返回上一步\n")
		if choose_in_manage not in {'0','1','2','3','9'}:
			...
			if choose_in_manage=='0':
				...
				self.main()
	def drop_online(self): #下线设备界面
		Print().SkyBlue_print("当前位置：首页>>设备管理>>下线设备")
		Print().Green_print("*"*50)
		i=0
		if online_ip!=[]:
			for ip in online_ip:
				Print().Green_print("-"*50)
				print("下线本设备请按："+str(i+1))
				print("设备ip："+ip)
				print("设备mac地址："+online_mac[i])
				print("设备名称："+online_name[i])
				if online_bind_flag[i]=='1':
					...
					print("该设备未绑定mac（原则上未来需重新认证）")
				i=i+1
			choose_in_drop_online=input("请按数字键选择您的操作\n0、退出程序\n8、全部下线\n9、返回上一步\n")
			check=0
			...
			if (choose_in_drop_online not in {'0','8','9'}) and (check>len(online_ip)):
				...
				if choose_in_drop_online=='0':
					...
					if tmp_ip in online_ip:
						Print().Green_print("下线ip地址为"+tmp_ip+"的设备失败，请稍后重试")
					else:
						Print().Green_print("下线ip地址为"+tmp_ip+"的设备成功")	
					self.drop_online()
		else:
			Print().Green_print("当前无任何设备在线")
			self.manage()
	def auth(self): #设备认证界面
		Print().SkyBlue_print("当前位置：首页>>设备管理>>设备认证")
		choose_in_auth=input("请按数字键选择您的操作\n0、退出程序\n1、本机未来免密认证\n2、本机未来需密认证\n3、非本机未来免密认证\n4、非本机未来需密认证\n9、返回上一步\n")
		if choose_in_auth not in {'0','1','2','3','4','9'}:
			...
			if choose_in_auth=='0':
				...
				Print().SkyBlue_print("首页>>设备管理>>设备认证>>非本机未来免密认证")
				ip_need_auth=input("请输入您想要认证设备的ip地址，例：172.20.22.66\n批量认证时ip之间用分号隔开，例：172.20.22.1;172.20.22.2;172.20.22.3\n")
				...
				for ip in ip_need_auth:
					...
					if check==None:
						Print().Red_print(ip+"输入有误，该ip无效，不会发送认证请求")
						self.auth()
					else:
				...
				Print().SkyBlue_print("首页>>设备管理>>设备认证>>非本机未来需密认证")
				ip_need_auth=input("请输入您想要认证设备的ip地址，例：172.20.22.66\n批量认证时ip之间用分号隔开，例：172.20.22.1;172.20.22.2;172.20.22.3\n")
				ip_need_auth=ip_need_auth.replace('；',';').strip()
				ip_need_auth=ip_need_auth.split(";")
				for ip in ip_need_auth:
				...
				self.main()
	def unbind(self): #解绑设备界面
		Print().SkyBlue_print("当前位置：首页>>设备管理>>解绑设备")
		...
		for ip in online_ip:
			if online_bind_flag[m]=='0':
				...
				print("解绑本设备请按："+str(i+1))
				print("设备ip："+ip)
				print("设备mac地址："+online_mac[m])
				print("设备名称："+online_name[m])
		...
		for ip in offline_ip:
			if offline_bind_flag[m]=='0':
				...
				print("解绑本设备请按："+str(i+1))
				print("设备ip："+ip)
				print("设备mac地址："+offline_mac[m])
				print("设备名称："+offline_name[m])
				unbindable_mac.append(offline_mac[m])
				Print().Green_print("-"*50)
				i=i+1
				m=m+1
		if unbindable_mac!=[]:
			choose_in_unbind_mac=input("请按数字键选择您的操作\n0、退出程序\n8、全部解绑\n9、返回上一步\n")
			check=0
			...
			if (choose_in_unbind_mac not in {'0','8','9'}) and (check>len(unbindable_mac)):
				...
				if choose_in_unbind_mac=='0':
					...
					if len(not_binded_mac)==len(online_mac)+len(offline_mac):
						...
						Print().Green_print("解绑全部mac失败，请稍后重试")
					self.manage()
					...
					if tmp_mac in not_binded_mac: 
						...
						Print().Green_print("解绑mac地址为"+tmp_mac+"的设备失败，请稍后重试")
					self.unbind()
		else:
			Print().Green_print("当前未绑定任何设备")
			self.manage()
	def rename(self): #重命名界面
		Print().SkyBlue_print("当前位置：首页>>设备管理>>设备重命名")
		global renameable_mac
		renameable_mac=[]
		i=0
		m=0
		for ip in online_ip:
			print("重命名本设备请按："+str(i+1))
			print("设备ip："+ip)
			print("设备mac地址："+online_mac[m])
			print("设备名称："+online_name[m])
		...
		for ip in offline_ip:
			...
			m=m+1
		if renameable_mac!=[]:
			choose_in_rename=input("请按数字键选择您的操作\n0、退出程序\n9、返回上一步\n")
			check=0
			...
			if (choose_in_rename not in {'0','9'}) and (check>len(renameable_mac)):
				...
				if choose_in_rename=='0':
					...
					new_name=input("请输入新的设备名\n")
					Command().rename(renameable_mac[t],new_name,renameable_bind_flag[t])
					Print().Green_print("已成功重命名mac地址为"+renameable_mac[t]+"的设备")
					Print().wait_time_axis(5)
					Login().get_status()
					self.rename()
def main_zh_cn(): #主函数
